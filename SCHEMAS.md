...
**Collections:** `org.hypercerts.claim.activity`, `org.hypercerts.claim.contribution`, `org.hypercerts.claim.contributorInformation`, `org.hypercerts.claim.rights`, `org.hypercerts.collection`, `org.hypercerts.context.acknowledgement`, `org.hypercerts.context.attachment`, `org.hypercerts.context.evaluation`, `org.hypercerts.context.measurement`, `org.hypercerts.feature`, `org.hypercerts.funding.receipt`, `org.hypercerts.tag`, `org.hypercerts.workscope.tag`
...
| `tags`                   | `ref[]`  | ❌       | References to org.hypercerts.tag records classifying this record. All listed tags apply simultaneously (logical AND); the array carries no ordering, weighting, negation, inheritance, or rule logic, and never will — any future expression logic must arrive as a new field and must never reinterpret this one. | maxLength: 20                                                               |
| `createdAt`              | `string` | ✅       | Client-declared timestamp when this record was originally created                                                                                                                                                                                                                                                  |                                                                             |
| `signatures`             | `ref`    | ❌       | Optional cryptographic signatures attesting to this record's content.                                                                                                                                                                                                                                              |                                                                             |
...
**Description:** A record for a subject that Hypercerts records can describe, such as a zone or stratum. It carries the subject's identity, classification, and optional published spatial representations. Geometry itself lives in app.certified.location records, and grouping lives in org.hypercerts.collection records.
...
| Property      | Type       | Required | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Comments                                       |
| ------------- | ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------- |
| `type`        | `string`   | ❌       | The coarse kind of subject this feature is, for example zone (an area a steward authors and manages) or stratum (an analytical unit derived from a model or methodology). This is not the feature's role or detailed classification — those are expressed through tags. Values beyond the known set are permitted.                                                                                                                                                                                                                                                 | maxLength: 64, Known values: `zone`, `stratum` |
...
| `locations`   | `ref[]`    | ❌       | Optional published spatial representations of this feature. Each referenced record must conform with app.certified.location. Multiple entries are alternative representations of the same subject at different precisions or encodings — for example an exact polygon and a coarser public representation for a sensitive site — never different places. Genuinely different places are either one MultiPolygon inside a single location record, or separate features. An empty or absent array is valid: the subject's geometry is unpublished or not applicable. | maxLength: 10                                  |
| `tags`        | `ref[]`    | ❌       | References to org.hypercerts.tag records classifying this record. All listed tags apply simultaneously (logical AND); the array carries no ordering, weighting, negation, inheritance, or rule logic, and never will — any future expression logic must arrive as a new field and must never reinterpret this one.                                                                                                                                                                                                                                                 | maxLength: 20                                  |
| `sameAs`      | `string[]` | ❌       | URIs of external identifiers that denote the same real-world subject, for example a cadastral parcel ID or a gazetteer entry. Entity identity only; vocabulary concordance belongs on org.hypercerts.tag.                                                                                                                                                                                                                                                                                                                                                          | maxLength: 20                                  |
| `createdAt`   | `string`   | ✅       | Client-declared timestamp when this record was originally created.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |                                                |
| `signatures`  | `ref`      | ❌       | Optional cryptographic signatures attesting to this record's content.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |                                                |
...
| `broader`           | `ref[]`    | ❌       | Optional references to directly broader terms, forming a polyhierarchy (a term may have several broader terms). Each referenced record must conform with org.hypercerts.tag.                                                                                                                                           | maxLength: 20                                                                                                                                        |
...
| `supersededBy`      | `ref`      | ❌       | The replacement term for a deprecated term, letting consumers roll classifications forward without rewriting published records. The referenced record must conform with org.hypercerts.tag.                                                                                                                            |                                                                                                                                                      |
